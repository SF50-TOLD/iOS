...
struct OpenMeteoConditionsTests {
  ...
  @Test
  func `reads the hour covering the requested time, in the units requested`() throws {
  ...
  @Test
  func `reads the hour a time falls on the start of, not the one it ends`() throws {
  ...
  @Test
  func `has no conditions for a time the forecast doesn’t reach`() throws {
...
struct OpenMeteoWindsAloftTests {
  ...
  @Test
  func `places each level at its geopotential height, lowest first`() throws {
  ...
  @Test
  func `drops levels the model puts below its own terrain, and levels missing a wind`() throws {
    let forecast = try #require(
      WindsAloftForecast(openMeteo: try Forecast.response, at: Forecast.firstHour)
    )
  ...
  @Test
  func `applies to the single hour it was forecast for, and says where it came from`() throws {
    let forecast = try #require(
      WindsAloftForecast(openMeteo: try Forecast.response, at: Forecast.firstHour)
    )
  ...
  @Test
  func `carries each level’s wind and temperature`() throws {
    let forecast = try #require(
      WindsAloftForecast(openMeteo: try Forecast.response, at: Forecast.firstHour)
    )
  ...
  @Test
  func `has no winds aloft for a time the forecast doesn’t reach`() throws {
    let response = try Forecast.response
...
struct OpenMeteoAtmosphericProfileTests {
  ...
  @Test
  func `reads cloud cover and humidity as fractions, at each level’s geopotential height`() throws {
  ...
  @Test
  func `drops levels the model puts below its own terrain, but keeps ones missing a wind`() throws {
    let profile = try #require(
      AtmosphericProfile(
        openMeteo: try Forecast.response,
        at: Self.coordinate,
  ...
  @Test
  func `carries the lowest and highest levels beyond the range they were reported over`() throws {
    let profile = try #require(
      AtmosphericProfile(
        openMeteo: try Forecast.response,
        at: Self.coordinate,
  ...
  @Test
  func `interpolates between the levels bracketing an altitude`() throws {
    let profile = try #require(
      AtmosphericProfile(
        openMeteo: try Forecast.response,
        at: Self.coordinate,
  ...
  @Test
  func `decodes the list a multi-coordinate request answers with`() throws {
  ...
  @Test
  func `has no profile for a time the forecast doesn’t reach`() throws {
    let response = try Forecast.response
    #expect(
...
struct OpenMeteoForecastWindowTests {
  ...
  @Test
  func `answers a time past the forecast horizon without asking the service`() async throws {
...
struct ConditionsMergeTests {
  ...
  @Test
  func `keeps every reported value and takes only the missing ones`() {
  ...
  @Test
  func `keeps the report’s own valid period and credits both services`() {
    let merged = partialReport.filling(from: modelForecast)
  ...
  @Test
  func `is complete only once nothing is left to fill`() {
    #expect(!partialReport.isComplete)
    #expect(partialReport.filling(from: modelForecast).isComplete)
  }

  @Test
  func `leaves a variable wind without a direction rather than inventing one`() {
  ...
  @Test
  func `takes a direction for a wind that wasn’t reported at all`() {
