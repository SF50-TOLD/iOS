...
///
/// A comparison spanning two models — a clean run against a contaminated one, one altitude
/// against the next — pairs each distance with the case that computed it, so no distance is
/// ever stamped with inputs it didn't come from.
///
...
struct PerformanceCase {
  ...
  private static let header = [
    ...
    "contamination_depth_in", "runway_condition_code", "result", "value_ft", "expected_ft"
  ]
  ...
  /// Expects `condition` to hold of distances that came from different models, attaching a row
  /// per distance so each carries the inputs that produced it.
  /// - Parameters:
  ///   - condition: What must hold across the distances.
  ///   - comment: What the condition means, e.g. `"Snow should lengthen the landing run"`.
  ///   - results: Each distance, keyed by what it is, paired with the case it came from.
  ///   - sourceLocation: The failing expectation's location.
  static func expect(
    _ condition: Bool,
    _ comment: Comment,
    results: KeyValuePairs<String, ComputedDistance>,
    sourceLocation: SourceLocation = #_sourceLocation
  ) {
    if !condition {
      attach(
        rows: results.map { result in
          result.value.performanceCase.row(
            result: result.key,
            value: field(result.value.distanceFt),
            expected: ""
          )
        }
      )
  ...
  private static func attach(rows: [[String]]) {
    Attachment.record(
  ...
  func expect(
    ...
    if !matches {
      Self.attach(rows: [row(result: result, value: field(actualFt), expected: field(expectedFt))])
    }
    #expect(
      matches,
  ...
  /// Expects `condition` to hold of distances this one case computed, attaching it and those
  /// distances when it doesn't. Distances that came from another model need the type-level
  /// overload instead, which names the case each of them came from.
  func expect(
    _ condition: Bool,
    _ comment: Comment,
    ...
    if !condition {
      Self.attach(rows: results.map { row(result: $0.key, value: field($0.value), expected: "") })
    }
    #expect(condition, comment, sourceLocation: sourceLocation)
  }
  ...
  func fail(
    ...
    Self.attach(rows: [row(result: result, value: "", expected: "")])
    Issue.record(comment, sourceLocation: sourceLocation)
  }

  /// Pairs a distance with the case that computed it, for a comparison spanning two models.
  /// - Parameter distanceFt: The distance this case's model computed.
  func computed(_ distanceFt: Double) -> ComputedDistance {
    .init(performanceCase: self, distanceFt: distanceFt)
  }
  ...
  /// A distance in feet alongside the case whose inputs produced it.
  struct ComputedDistance {
...
extension DataTable {
  ...
  private func attachRows() {
    ...
    let header = (0..<inputs(from: firstRow).count).map { "input_\($0 + 1)" } + ["output"]
    Attachment.record(
