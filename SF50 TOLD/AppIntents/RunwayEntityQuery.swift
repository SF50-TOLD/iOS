...
extension RunwayEntityQuery {
  /// The airport whose runways the picker offers.
  ///
  /// The airport is read off the intent as ``RunwayNumbersIntent/scopedAirportRecordID`` rather than
  /// as the `airport` parameter, because an `IntentProjection` hands back a parameter's *unwrapped*
  /// type: projecting an `AirportEntity?` the person left empty would unwrap nothing at all. Only the
  /// case where there is no intent in play at all — Spotlight, say — is left to the app's own
  /// selection here.
  private var airportRecordID: String? {
    guard let context else { return Defaults[.takeoffAirport] }
    let scoped = context.scopedAirportRecordID
