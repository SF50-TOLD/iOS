...
struct RunwayNumbersIntent: AppIntent {
    ...
    -> some ReturnsValue<RunwayEntity> & ShowsSnippetIntent & ProvidesDialog
  {
    ...
    guard let name = runway?.name ?? Defaults[Self.runwaySelectionKey(for: operation)] else {
...
extension RunwayNumbersIntent {
  ...
  private static func runwaySelectionKey(
    for operation: SF50_Shared.Operation
  ) -> Defaults.Key<String?> {
    switch operation {
      case .takeoff: .takeoffRunway
      case .landing: .landingRunway
    }
  }
